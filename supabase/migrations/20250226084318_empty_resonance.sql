/*
  # Fix innings and balls setup

  1. Changes
    - Add validation to ensure innings exists before ball submission
    - Add proper cleanup of existing triggers and policies
...
DROP POLICY IF EXISTS "match_innings_read_policy_v4" ON match_innings;
DROP POLICY IF EXISTS "match_innings_write_policy_v4" ON match_innings;

-- Create function to validate ball submission
CREATE OR REPLACE FUNCTION validate_ball_submission_v5()
RETURNS trigger AS $$
DECLARE
  v_innings RECORD;
BEGIN
...
CREATE OR REPLACE FUNCTION update_innings_on_ball_v5()
RETURNS trigger AS $$
DECLARE
...
CREATE TRIGGER validate_ball_submission_trigger_v5
  BEFORE INSERT ON balls
  FOR EACH ROW
  EXECUTE FUNCTION validate_ball_submission_v5();

CREATE TRIGGER update_innings_on_ball_trigger_v5
  AFTER INSERT ON balls
  FOR EACH ROW
  EXECUTE FUNCTION update_innings_on_ball_v5();

-- Create policies with unique names
CREATE POLICY "match_innings_read_policy_v5"
  ON match_innings FOR SELECT
  TO public
  USING (true);

CREATE POLICY "match_innings_write_policy_v5"
...
CREATE POLICY "balls_read_policy_v5"
  ON balls FOR SELECT
  TO public
  USING (true);

CREATE POLICY "balls_write_policy_v5"
  ON balls FOR ALL
  TO authenticated
  USING (
    EXISTS (

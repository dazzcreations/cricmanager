/*
  # Fix innings and balls handling

  1. Changes
    - Add match_id to balls table
...
DROP POLICY IF EXISTS "match_innings_read_policy" ON match_innings;
DROP POLICY IF EXISTS "match_innings_write_policy" ON match_innings;

-- Add match_id to balls if not exists
DO $$ 
BEGIN
  ALTER TABLE balls
...
CREATE OR REPLACE FUNCTION validate_ball_submission_v4()
RETURNS trigger AS $$
DECLARE
  v_innings RECORD;
BEGIN
...
CREATE OR REPLACE FUNCTION update_innings_on_ball_v4()
RETURNS trigger AS $$
DECLARE
...
-- Create triggers
CREATE TRIGGER validate_ball_submission_trigger_v4
  BEFORE INSERT ON balls
  FOR EACH ROW
  EXECUTE FUNCTION validate_ball_submission_v4();

CREATE TRIGGER update_innings_on_ball_trigger_v4
  AFTER INSERT ON balls
  FOR EACH ROW
  EXECUTE FUNCTION update_innings_on_ball_v4();

-- Create policies with unique names
CREATE POLICY "match_innings_read_policy_v4"
  ON match_innings FOR SELECT
  TO public
  USING (true);

CREATE POLICY "match_innings_write_policy_v4"
...
CREATE POLICY "balls_read_policy_v4"
  ON balls FOR SELECT
  TO public
  USING (true);

CREATE POLICY "balls_write_policy_v4"
  ON balls FOR ALL
  TO authenticated
  USING (
    EXISTS (
